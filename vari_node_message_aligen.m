function [ new_Vmapping,pda_join_x_z,t_z_transform  ] = vari_node_message_aligen( Vmapping,channel_dist,node_distri,T )
...
for ii=1:length(node_distri)
    if(node_distri(ii)~=0&&ii~=base_degree)
        ...
        if ii==1 %%(channel observation)
            [ pd_join_x_z,t_z_transform(ii,:) ] = message_alignment( channel_dist,pda_join_x_z,T);
        else            
