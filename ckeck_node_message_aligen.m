...
for ii=1:length(node_distri)
    if(node_distri(ii)~=0&&ii~=base_degree)
        ...
        [ pd_join_x_z,pd_z_given_t ] = message_alignment( Cmapping(ii-2).NProbJoinXT,pda_join_x_z,T);
        [ pd_z_given_y ] = mapping_update( pd_z_given_t,Cmapping(ii-2).ProbConTT1T2 );
        ...
        new_Cmapping(ii-2).NProbJoinXT=pd_join_x_z;     %update new_cmapping
        new_Cmapping(ii-2).ProbConTT1T2=pd_z_given_y;   %update new_cmapping
        for kk=1:T
